...
- **日出日落本地计算** —— NOAA 太阳位置算法，不联网。坐标可以来自系统定位服务，
  也可以搜城市或手填经纬度；都没有时，按系统时区反查一个近似坐标，免权限也能用。
  中国全境都是 `Asia/Shanghai`，选深圳还是上海，日出能差十几到四十分钟
- **自动更新位置** —— 新配置默认开启，授权后每天及切换时区时检查，失败保留上次位置。
  选择城市或输入坐标会关闭自动更新；升级时保留已有固定位置。位置页显示上次成功检查时间。
...
- **它自己的文件。** 日程、引擎状态与导入的壁纸组在 `~/Library/Application Support/HourGlow/`；
  ...
  磁盘上别的地方一概不碰。
- **权限。** 一项都不是必需的。定位是可选的：授权后以明文经纬度存进 `schedule.json`，
  菜单栏 app 运行时默认每天自动检查，可在位置页关闭；不给就搜城市或手填经纬度，再不然按时区推断。开机自启是普通的登录项，
...
- **哪些会联网。** 自动更新开着（默认）时，每天向 `api.github.com` 问一次最新版本；
  你点了安装才从 GitHub 下载。搜索地点时，内置城市表有结果就保持离线；没有结果且至少输入两个字符时，
  会在短暂停顿后向 Apple MapKit 查询。CLI 离线查找无结果时也使用 MapKit，不再请求 Nominatim。
  系统定位使用 macOS 定位服务；位置变化或缺少名称时，
  app 还会通过 MapKit 查询地名。自动更新位置与版本检查有各自的开关。没有遥测，没有统计，不需要账号。
