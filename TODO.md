...
- [x] 导入改成两阶段提交：新素材写进唯一目录，配置成功落盘后才按磁盘上的最新时间轴
      清理旧目录；复制 / 保存失败可以撤掉新目录，同名或并发导入不会破坏仍在引用的素材
- [x] 更新器边读取边等待 `ditto` / `codesign`，避免大量错误输出写满 pipe 后父子进程互等
- [x] 相同触发时刻按配置顺序稳定决胜（靠后的生效），且「下一次」预告与到点结果一致
- [x] app 与 CLI 抢不到 `EngineLock` 时保持从属并定期重试；原领跑者退出或 LaunchAgent
      被卸载后自动接管，不再留下一个仍显示运行、实际无人排程的进程
- [x] `modelcheck` / `enginecheck` / `importcheck` / `updatecheck` 已补上述回归覆盖
